...
3. Hook 350–500 символов: боль → ответ → **что сделает читатель**.
4. Минимум **5** нумерованных шагов `<ol>` + workflow (`→`) или таблица.
5. Каждый H2: рекомендация «делать / не делать».
6. Body 8 500–9 500 знаков, FAQ 5–7 пар — **ответы-действия**.
7. `article.meta.json`: `article_mode: B`, `char_count`, `meta_ab` с A/B/AEO оптимизацией:
...
8. Подзаголовки H2 должны начинаться с **глаголов действия** (создайте, настройте, проверьте) или содержать **маркеры действия** (как, чеклист, сравнение, выбор, пошагово). Минимум 3 H2 должны быть активными.
